...
const deserializeStartList = (row: any): StartList | null => {
    ...
    return {
        ...
        hasPendingPhotos: parseBool(row.get('hasPendingPhotos')),
    };
};
...
/**
 * Targeted update wybranych pól Start Listy (np. podmiana placeholderów zdjęć po sync).
 * Nie nadpisuje całego wiersza — odporne na równoległą edycję innych pól.
 */
export async function updateStartListFields(
    addressId: string,
    updates: Partial<Omit<StartList, 'addressId'>>
): Promise<void> {
    const sheet = await getSheet(SHEET_NAME_START_LISTS, START_LIST_HEADERS);
    const rows = await withTimeout(sheet.getRows(), TIMEOUT_MS, 'sheet.getRows(StartLists)');
    const row = rows.find(r => r.get('addressId') === addressId);
    if (!row) throw new Error('Start list not found');
    const keys = Object.keys(updates) as Array<keyof typeof updates>;
    for (const key of keys) {
        const value = updates[key];
        if (value === undefined) continue;
        if (key === 'hasBalcony' || key === 'hasPendingPhotos') {
            row.set(key, (value as boolean) ? 'TRUE' : 'FALSE');
        } else if (key === 'transport' || key === 'kitchenPhotoUrls' || key === 'bathroomPhotoUrls' || key === 'roomsPhotoUrls' || key === 'hallwayPhotoUrls') {
            row.set(key, JSON.stringify(value));
        } else {
            row.set(key, value as string);
        }
    }
    await withTimeout(row.save(), TIMEOUT_MS, 'row.save(StartLists)');
    startListsCache = null;
}
