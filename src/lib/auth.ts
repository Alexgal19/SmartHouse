...
export async function login(name: string, password_input: string) {
  ...
  if (name.toLowerCase() === 'admin' && password_input === process.env.ADMIN_PASSWORD) {
    ...
    await session.save();
    return { success: true, user: { uid: 'admin-hardcoded', name: 'Admin', isAdmin: true, isDriver: false, isRekrutacja: false, isBok: false, isGuest: false, canEditPastControlCards: true } };
  }
  ...
  if (user) {
    ...
    await session.save();
    return { success: true, user: { uid: user.uid, name: user.name, isAdmin: user.isAdmin, isDriver: user.isDriver || false, isRekrutacja: user.isRekrutacja || false, isBok: user.isBok || false, isGuest: user.isGuest || false, canEditPastControlCards: user.canEditPastControlCards || false } };
  }
