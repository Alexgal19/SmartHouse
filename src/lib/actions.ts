...
        if (importedCount > 0) {
            const notificationSheet = await getSheet(SHEET_NAME_NOTIFICATIONS, ['id', 'message', 'createdAt']);
            console.log(`Successfully imported ${importedCount} employees. Notification skipped for simplicity.`);
        }
        
        return { success: true, message: `Успішно імпортовано ${importedCount} співробітників. З помилками: ${errorCount}.` }; 

    } catch (error) {
        console.error("Error in bulkImportEmployees:", error);
        return { success: false, message: `Критична помилка імпорту: ${error instanceof Error ? error.message : 'Unknown error'}. Помилки у рядках: ${errors.join('; ')}` };
    }
}
