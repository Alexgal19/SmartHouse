...
export async function updateCoordinatorSubscription(coordinatorId: string, subscription: string | null): Promise<void> {
    try {
        ...
        if (coordinatorRow) {
            ...
            await Promise.all([withTimeout(coordinatorRow.save(), TIMEOUT_MS, 'row.save(Coordinators)'), invalidateSettingsCache()]);
            revalidatePath('/dashboard');
        } else {
        ...
        if (
            ...
            try {
                ...
                if (coordRow) {
                    ...
                    await Promise.all([withTimeout(coordRow.save(), TIMEOUT_MS, 'row.save(Coordinators)'), invalidateSettingsCache()]);
                }
            } catch (cleanupErr) {
